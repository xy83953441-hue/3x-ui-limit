...
// CheckClientIpJob monitors client IP addresses from access logs and
// enforces per-client device limits by removing over-limit clients from
// Xray. Works identically on all platforms without external tools.
...
func (j *CheckClientIpJob) updateInboundClientIps(inboundClientIps *model.InboundClientIps, clientEmail string, newIpsWithTime []IPWithTimestamp) bool {
	...
	if len(liveIps) > limitIp {
		...
		// Disconnect the client (RemoveUser) and clear its IP records.
		// Unlike the old fail2ban approach — which re-added the user and
		// relied on fail2ban to block excess IPs — this method removes
		// the user entirely. On the next tick (10s), if the client
		// reconnects with ≤ limitIp IPs they stay connected; if still
		// over the limit they get removed again. Works identically on
		// all platforms without external tools.
		j.disconnectAndClearClient(inbound, clientEmail, clients)

		// log the event for admin visibility (no fail2ban dependency)
		logger.Warningf("[LIMIT_IP] Client %s disconnected (exceeded limit of %d): %d live IPs seen",
			clientEmail, limitIp, len(liveIps))

		// Skip IP record save — disconnectAndClearClient already cleared them.
		// Next tick starts fresh from the access log.
		return shouldCleanLog
	} else {
		keptLive = liveIps
	}
	...
	logger.Debugf("[LIMIT_IP] Client %s: %d live + %d historical IPs recorded",
		clientEmail, len(keptLive), len(historicalIps))

	return shouldCleanLog
}

// disconnectAndClearClient removes the client from Xray (kicking all
// connections) and clears its tracked IP records. The old re-add + fail2ban
// approach is replaced by pure Xray API removal — on the next job tick
// (10s) the client's new connections are re-evaluated from the access log.
func (j *CheckClientIpJob) disconnectAndClearClient(inbound *model.Inbound, clientEmail string, _ []model.Client) {
	...
	default:
		logger.Warningf("[LIMIT_IP] Disconnect not supported for protocol %s on inbound %s", inbound.Protocol, inbound.Tag)
		return
	}

	// Remove user — all its connections drop immediately
	...
	// Clear the client's IP records so the next tick starts fresh
	j.clearClientIps(clientEmail)

	logger.Infof("[LIMIT_IP] Client %s removed from inbound %s and IP records cleared", clientEmail, inbound.Tag)
}

// clearClientIps clears the tracked IPs for a client by email.
func (j *CheckClientIpJob) clearClientIps(clientEmail string) {
	db := database.GetDB()
	err := db.Model(model.InboundClientIps{}).
		Where("client_email = ?", clientEmail).
		Update("ips", "").Error
	if err != nil {
		logger.Warningf("[LIMIT_IP] Failed to clear IPs for %s: %v", clientEmail, err)
	}
}
